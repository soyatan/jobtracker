...
export default AddMeetingModal=({modalVisible,setModalVisible,text, state})=>{
  ...
  const[meetingCounter,setMeetingCounter]=useState(text.length)
...
return(
        <View style={styles.meetingcontainer}>
          <View style={styles.meetingheader}><Text style={{fontWeight:'bold'}}>Meeting No:{item.item.id} </Text>
          <TouchableOpacity onPress={()=>deleteMeeting(item.item.id)} style={styles.trashiconcontainer}>
          <FontAwesome name='trash' size={24} />
          </TouchableOpacity>
        </View>
          
          <MeetingDateInput title={'Date'} text={findMeeting(item.item.id).date} state={setMeetingDate} id={item.item.id} add={addMeetingDates}/>
          <MeetingNoteInput title={'Notes'} text={findMeeting(item.item.id).meetingnotes} state={setMeetingNotes} id={item.item.id} add={addMeetingNotes}/>
          </View>  ) }}
    >      
