...
export default FilterItem=({item,filterValues,setActiveFilter,mainFilter,setMainFilter,activeFilter})=>{
 ...
 useEffect(() => {
  ...
  const toggleChoice = () =>{
    if(isSelected)
      ...
      const filteredGroup=newMainFilter[0][activeFilter].filter(e=>e!==item)
  ...
  return(
        ...
        <TouchableOpacity onPress={()=>toggleChoice()} style={styles.filtermodalbody}>
        <Text style={styles.header}>{item}</Text>
        {isSelected?
        <Tick width={styles.icondimensions.width} height={styles.icondimensions.height}/>  :null}
